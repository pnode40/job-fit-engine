...
# Replace states
old_states = """  const [dossierContent, setDossierContent] = useState("");

  const [jobDescription, setJobDescription] = useState("");"""
new_states = """  const [dossierContent, setDossierContent] = useState("");
  const [jobDescription, setJobDescription] = useState("");
  const [activeStep, setActiveStep] = useState(1);"""

content = content.replace(old_states, new_states)

# Replace getStep and currentStep
old_getstep = """  const getStep = () => {
    if (documents) return 4;
    if (evaluation) return 3;
    if (jobDescription.length > 50) return 2;
...
new_getstep = """  const unlockedStep = () => {
    if (documents) return 4;
    if (evaluation) return 3;
...
"""
content = content.replace(old_getstep, new_getstep)

# Replace the return block
old_return_pattern = re.compile(r"return \([\s\S]*?\);\n}\n?$", re.MULTILINE)
              ...
              >
                ...
                <Tabs defaultValue="resume" className="w-full">
                  ...
                  <TabsContent value="resume" className="space-y-4">
                    ...
                    <div className="hidden"><div ref={resumeRef} className="print-container p-12 bg-white text-black font-sans max-w-[800px] mx-auto"><style type="text/css" media="print">{`@page { size: letter; margin: 0.5in; } .print-container { font-size: 11px; line-height: 1.4; }`}</style><Markdown>{documents.resumeMarkdown}</Markdown></div></div>
