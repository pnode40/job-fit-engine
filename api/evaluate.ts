...
7. EXPLICIT DISQUALIFIER RULE (HARD WALL — overrides Rule 8): When a JD explicitly states that a specific tool, platform, credential, or type of experience "does not qualify," "does not count," "is not sufficient," or uses equivalent disqualifying language for a given requirement, you MUST treat the candidate's possession of that exact thing as contributing ZERO credit toward that requirement. This is a non-negotiable hard wall. The JD author has already made the adjacency judgment and ruled it out — you may not second-guess it. Do not award partial credit for explicitly disqualified items. Each such disqualified must-have MUST be counted in mustHaveGapCount and listed as a badFitReason. Example: A JD that says "Must have: corporate LMS experience (Cornerstone, SAP SuccessFactors, Workday Learning). K-12 platforms such as Canvas LMS, Google Classroom, and Nearpod do not qualify" means a candidate whose only LMS experience is Canvas LMS has a must-have gap — full stop, zero partial credit.
8. MUST-HAVE GAP COUNT: After applying the Explicit Disqualifier Rule above, count the number of requirements in the JD that are explicitly stated as mandatory, required, or must-have that the candidate does NOT meet (including those blocked by Rule 7). Return this as mustHaveGapCount.
9. TRANSFERABLE SKILLS & ADJACENT EXPERIENCE: When a candidate lacks a specific requirement but has demonstrably adjacent experience, evaluate whether that experience reasonably bridges the gap. For example: K-12 curriculum design is relevant (not equivalent) to corporate instructional design. Quota-carrying B2B sales is relevant (not equivalent) to sales enablement. Score adjacent experience as partial credit — above zero but below what direct experience would receive. The exact score should reflect how strong the transfer logic is for the specific case. However, if the candidate's entire background is in an unrelated field with no demonstrable connection to the target role's core function, this logic does not apply — score based on actual evidence only. Important: Even when a JD explicitly welcomes career pivots or prefers adjacent backgrounds, a candidate making their first move into a new functional role — with no prior job title, formal program, or measurable output in that function — should receive dimensional scores that reflect the actual gap, not the JD's hiring intent. A JD that says it 'prefers' or 'welcomes' pivot candidates is describing hiring intent, not a waiver of the experience gap. Adjacent experience reduces the gap; it does not eliminate it. This rule does NOT apply to items that were explicitly disqualified by Rule 7.
